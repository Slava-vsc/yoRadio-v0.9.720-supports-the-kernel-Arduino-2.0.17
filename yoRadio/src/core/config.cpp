...
bool Config::loadStation(uint16_t ls) {
  ...
  if (cs == 0) {
    ...
    strncpy(station.name, "інтернет радіо", BUFLEN);
    station.ovol = 0;
    return false;
  }
...
void Config::bootInfo() {
  ...
  BOOTLOG("------------------------------------------------");
  BOOTLOG("arduino:\t%d", (int)ARDUINO);
  BOOTLOG("compiler:\t%s", __VERSION__);
  #ifdef ESP_ARDUINO_VERSION_MAJOR
    BOOTLOG("esp32core:\t%d.%d.%d", (int)ESP_ARDUINO_VERSION_MAJOR, (int)ESP_ARDUINO_VERSION_MINOR, (int)ESP_ARDUINO_VERSION_PATCH);
  #else
    BOOTLOG("esp32core:\tLegacy / Unknown");
  #endif
  BOOTLOG("chip:\t\tmodel: %s | rev: %d | id: %u", 
          ESP.getChipModel(), 
  ...
  BOOTLOG("Flash:\t\t%d MB", (int)(ESP.getFlashChipSize() / (1024 * 1024)));
  BOOTLOG("Flash Freq:\t%d MHz", (int)(ESP.getFlashChipSpeed() / 1000000));
  BOOTLOG("------------------------------------------------");
